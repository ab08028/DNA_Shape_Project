...
flagOfAnalysis=paste0(todaysdate,".plots.projectedCounts.epsilon.",epsilon,".sepCpGs.",separateCpGs,".addingApes_maskedMouseVaq.addingMultinomialDownsampling.MorePlotsForPAG") # a label that will be in title of plots and outdir 
...
# 
spectra_7mer_spectrumDistMat_ForMantel <- all7merSpectra %>%
  processSpectra(.) %>%
  merge(.,speciesCodes,by.x="label",by.y="code",suffixes=c(".spectrum",".sppCodes")) %>% 
  filter(label %in% speciesToInclude_phylo) %>% # need to prefilter spp otheriwse you get dups in spp name for diff pops (just for this mantel thing I'm doing)
  pivotSpectra_perVariable_KEEPFULLSPECIESNAMES("mutation_rate_projected_normalized_plusEpsilon") %>%
  clr_or_ilr_orNoTransform_calculations(.,"clr") %>%
  euclideanDistance_keepMatrix_UPPERANDLOWER(.)  # need to update species codes 
...
# smaller subet:
pivot3merForHeatmap_melt_subset_smaller <- pivot3merForHeatmap_melt_subset[pivot3merForHeatmap_melt_subset$variable %in% c("bears_PB_over_bears_ABC","apes_humans_AFR_over_mice_Mmd"),]

pivot3merForHeatmap_melt_subset_smaller$variable <- factor(pivot3merForHeatmap_melt_subset_smaller$variable,levels=c("bears_PB_over_bears_ABC","apes_humans_AFR_over_mice_Mmd"))
...
heatmap_niceVersion <- ggplot(pivot3merForHeatmap_melt_subset_smaller,aes(x=three_prime_flanking_base,y=five_prime_flanking_base,fill=value))+
  geom_tile()+
  facet_grid(~nicerCentralBPLabel~variable)+
  scale_fill_gradient2(high="red",mid="white",low="blue",midpoint = 1)+
  theme_bw()+
  geom_text(aes(label=mutation_label),size=2)
heatmap_niceVersion
ggsave(paste0(plotdir,"heatmap.2Comparisons.pdf"),heatmap_niceVersion,height=7,width=7)
