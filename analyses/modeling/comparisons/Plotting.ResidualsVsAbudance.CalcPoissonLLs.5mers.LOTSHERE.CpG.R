...
tail(truth_prediction_df[,c("mutationType","central5mer")])
# get the average outcome for the central 5mer rather than each 7mer
truth_prediction_df <- truth_prediction_df %>%
  group_by(population, central5mer,newGroup) %>%
  mutate(average5merRate=mean(outcome))

# plot the central 5mers and the predictions and see the relationship
# low abundance only (< 5 mutations)
truth_prediction_df$abundanceLabel <- "high"
truth_prediction_df[truth_prediction_df$mutationCount<=1,]$abundanceLabel <- "low"

ggplot(truth_prediction_df[truth_prediction_df$population=="Mmd" & truth_prediction_df$abundanceLabel=="low",],aes(x=central5mer,y=.pred))+
  geom_point(aes(x=central5mer,y=average5merRate),alpha=0.5,size=3,color="purple")+
  geom_point()+
  facet_wrap(~mutationLabel,scales="free")+
  ggtitle("super low abundance 7mers -- do their predictions match the average of their central 5mer?")

################# want to compare training outcomes of super low abundance 7mers to the final predictions and the final test data set ############

oneFoldSetToTrainAndAssessOn <- readRDS("/Users/annabelbeichman/Documents/UW/DNAShapeProject/results/modeling/experiments/20210704_model_012_RF.plusMethylInfo/oneFoldSetToTrainAndAssessOn.rds")
training <- training(oneFoldSetToTrainAndAssessOn)
training$central5mer <- paste0(substr(training$mutationType,2,6),".",substr(training$mutationType,10,14))
head(training)
training <- training %>%
  group_by(population,central5mer )  %>% 
  mutate(average5merRateAcrossAllTrainingWindows=mean(outcome)) 

# combine train and test
training$label2 <- "trainingFold"
truth_prediction_df$label2 <- "asssessmentFold"

# get avg of training windows
avgTrainingOutcomes <- training %>%
  group_by(population,mutationType) %>%
  summarise(avgOutcomeAcrossTrainingWindows=mean(outcome),avgMutationCountAcrossTrainingWindows=mean(mutationCount))
avgTrainingOutcomes
# want to compare this to the predictions

combo <- merge(avgTrainingOutcomes,truth_prediction_df[,c("mutationType","mutationCount",".pred","outcome","central5mer")])
head(combo) # here outcome is in the testing dataset (truth) and .pred is the prediction from model for the test dataset

# okay want to compare the training outcomes for super low abundance with the test set
lowabundance_melt <- melt(combo[combo$mutationCount<=1 | combo$avgMutationCountAcrossTrainingWindows<=1,])

#### does htis show that it's maybe ok?
ggplot(lowabundance_melt[lowabundance_melt$variable %in% c("avgOutcomeAcrossTrainingWindows",".pred","outcome"),],aes(x=mutationType,y=value,color=variable))+
  geom_point(size=.5)+
