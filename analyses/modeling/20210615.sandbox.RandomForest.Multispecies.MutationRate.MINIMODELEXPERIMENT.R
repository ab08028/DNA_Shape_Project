...
modelLabel="RF"
description=paste0(modelLabel,".",outcomeLabel,".Multispecies.DummyPopVar.TryingAMiniModel") # short description of these experiments
description
chromCount=19
...
##### SUBSET DATA AS EXPERIMENT: just chroms 1,2,3,4 (1,3= train, 2,4=test) ###########
allData_withShapes_unprocessed <- allData_withShapes_unprocessed[allData_withShapes_unprocessed$window %in% c(1,2,3,4),]
...
# started around 3:50pm on 6/15 on mini dataset (only training data from 1 chr)
