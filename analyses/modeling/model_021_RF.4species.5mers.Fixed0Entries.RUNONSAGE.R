...
description <- args[1]
outdir <- args[2]
#outdir="/Users/annabelbeichman/Documents/UW/DNAShapeProject/results/modeling/experiments/20210719_model_021_RF.4species.5mers.Fixed0Entries_runOnLaptop/"
...
allData_multipop <- read.table(paste0(spectrumdir,"TEMPORARY.NEWGROUPS.MULTIPOPULATION_spectrumCountsAndTargetCounts_perChromosome.allChrs.Labelled.INCLUDESCorrected0Entries.all4miceSpecies.txt"),header=T) # this is the 7mer spectrum but down below you are going to sum up over 5mers  # 20210719 this file has corrected 0 entries AND has all four mouse species! 
# for testing on home computer:
#allData_multipop <- read.table("/Users/annabelbeichman/Documents/UW/BearProject/results/mutyper/wild_mouse_data/mutyperResults_20210317_NOSTRICT_7mer/mutyper_spectrum_target_merged_PerChr/TEMPORARY.NEWGROUPS.MULTIPOPULATION_spectrumCountsAndTargetCounts_perChromosome.allChrs.Labelled.INCLUDESCorrected0Entries.all4miceSpecies.txt",header=T) # 20210719 this file has corrected 0 entries 
...
ggsave(paste0(outdir,"modelTrainedOnOneFold.PredictionsPlot.AssessedOnWindow",toString(unique(truth_prediction_df$newGroup)),".png"),rand_forest_Fold01_fit_predictions_plot,height=9,width=9)
...
ggsave(paste0(outdir,"modelTrainedOnOneFold.PredictionsPlot.FacetedPerMutationType.AssessedOnWindow",toString(unique(truth_prediction_df$newGroup)),".png"),rand_forest_Fold01_fit_predictions_plot_faceted,height=12,width=18)
...
# need to facet pairs of plots 
populations=c("Mmm","Mmd","Mmc","Ms")
populationCombos <- combn(populations,2)
totalCombos=dim(populationCombos)[2]
populationCombos
# save all pairs of plots:
########## NEED AN EFFICIENT WAY TO GET ALL COMBOS #######
# BUT FOR NOW

speciesComparisonPlot1 <- ggplot(truth_prediction_df_spread,aes(x=outcome_Mmd,y=outcome_Ms))+
  geom_point()+
  ...
  theme_bw()+
  ggtitle("Mmd vs Ms")
speciesComparisonPlot1
ggsave(paste0(outdir,"compare.Mmd.Ms.modelTrainedOnOneFold.SpeciesXYComparison.AssessedOnWindow",toString(unique(truth_prediction_df$newGroup)),".png"),speciesComparisonPlot1,height=5,width=7)

speciesComparisonPlot2 <- ggplot(truth_prediction_df_spread,aes(x=outcome_Mmd,y=outcome_Mmm))+
  geom_point()+
  geom_point(aes(x=.pred_Mmd,y=.pred_Mmm),shape=1,color="red")+
  geom_abline()+
  facet_wrap(~centralMutationType,scales="free")+
  theme_bw()+
  ggtitle("Mmd vs Mmm")
speciesComparisonPlot2
ggsave(paste0(outdir,"compare.Mmd.Mmm.modelTrainedOnOneFold.SpeciesXYComparison.AssessedOnWindow",toString(unique(truth_prediction_df$newGroup)),".png"),speciesComparisonPlot2,height=5,width=7)

speciesComparisonPlot3 <- ggplot(truth_prediction_df_spread,aes(x=outcome_Mmd,y=outcome_Mmc))+
  geom_point()+
  geom_point(aes(x=.pred_Mmd,y=.pred_Mmc),shape=1,color="red")+
  geom_abline()+
  facet_wrap(~centralMutationType,scales="free")+
  theme_bw()+
  ggtitle("Mmd vs Mmc")
speciesComparisonPlot3
ggsave(paste0(outdir,"compare.Mmd.Mmc.modelTrainedOnOneFold.SpeciesXYComparison.AssessedOnWindow",toString(unique(truth_prediction_df$newGroup)),".png"),speciesComparisonPlot3,height=5,width=7)


speciesComparisonPlot4 <- ggplot(truth_prediction_df_spread,aes(x=outcome_Ms,y=outcome_Mmc))+
  geom_point()+
  geom_point(aes(x=.pred_Ms,y=.pred_Mmc),shape=1,color="red")+
  geom_abline()+
  facet_wrap(~centralMutationType,scales="free")+
  theme_bw()+
  ggtitle("Ms vs Mmc")
speciesComparisonPlot4
ggsave(paste0(outdir,"compare.Ms.Mmc.modelTrainedOnOneFold.SpeciesXYComparison.AssessedOnWindow",toString(unique(truth_prediction_df$newGroup)),".png"),speciesComparisonPlot4,height=5,width=7)

speciesComparisonPlot5 <- ggplot(truth_prediction_df_spread,aes(x=outcome_Mmm,y=outcome_Mmc))+
  geom_point()+
  geom_point(aes(x=.pred_Mmm,y=.pred_Mmc),shape=1,color="red")+
  geom_abline()+
  facet_wrap(~centralMutationType,scales="free")+
  theme_bw()+
  ggtitle("Mmm vs Mmc")
speciesComparisonPlot5
ggsave(paste0(outdir,"compare.Mmm.Mmc.modelTrainedOnOneFold.SpeciesXYComparison.AssessedOnWindow",toString(unique(truth_prediction_df$newGroup)),".png"),speciesComparisonPlot5,height=5,width=7)


speciesComparisonPlot6 <- ggplot(truth_prediction_df_spread,aes(x=outcome_Mmm,y=outcome_Ms))+
  geom_point()+
  geom_point(aes(x=.pred_Mmm,y=.pred_Ms),shape=1,color="red")+
  geom_abline()+
  facet_wrap(~centralMutationType,scales="free")+
  theme_bw()+
  ggtitle("Mmm vs Ms")
speciesComparisonPlot6
ggsave(paste0(outdir,"compare.Mmm.Ms.modelTrainedOnOneFold.SpeciesXYComparison.AssessedOnWindow",toString(unique(truth_prediction_df$newGroup)),".png"),speciesComparisonPlot6,height=5,width=7)
