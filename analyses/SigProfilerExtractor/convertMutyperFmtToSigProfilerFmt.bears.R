...
input <- read.table(paste0(indir,"All_Intervals_CountsProps.ref.brown_bear.ALLFREQS.nostrict.txt"),header=T)
head(input)
...
head(input)
input_wider <- pivot_wider(select(input,-c(variable,proportionOfSites)),id_cols = c(`Mutation Types`,population),names_from =  population,values_from = totalSitesAllIntervals)
head(input_wider)                    


# 
write.table(input_wider,paste0(indir,"All_Intervals_Counts.perPopulation.ref.brown_bear.ALLFREQS.nostrict.CentralAsRevCompedToTs.SIGPROFILEREXTRACTORFORMAT.txt"),row.names=F,quote=F,sep="\t")
