...
## bears:
indir="/Users/annabelbeichman/Documents/UW/BearProject/results/mutyper/wild_mouse_data/mutyperResults_20210706_NOSTRICT_3mer_REPEATMASKED/summaryTables/"
speciesLabel="mice"
ksfs = read.table(paste0(indir,"/3merSFS.PerPopulation.REPEATMASKED.txt"),header=T,sep="\t")
head(ksfs)
# so I want to subset to singletons on
ly:
...
singletonsOnly_wider <- pivot_wider(select(singletonsOnly,-c(variable)),id_cols = c(`Mutation Types`,population),names_from =  population,values_from = sitesGenomeWide)
head(singletonsOnly_wider)                    


# 
write.table(singletonsOnly_wider,paste0(indir,"All_Intervals_kmerCountsForSigProfilerExtractor.SINGLETONSONLY.",speciesLabel,".txt"),row.names=F,quote=F,sep="\t")
...
names(ksfs)
ksfs_wider <- pivot_wider(select(ksfs,-c(variable)),id_cols = c(`Mutation Types`,population),names_from =  c(population,sample_frequency),values_from = sitesGenomeWide)
head(ksfs_wider)                    
write.table(ksfs_wider,paste0(indir,"All_Intervals_kmerCountsForSigProfilerExtractor.ALLFREQUENCIESSEPARATED.",speciesLabel,".txt"),row.names=F,quote=F,sep="\t")
