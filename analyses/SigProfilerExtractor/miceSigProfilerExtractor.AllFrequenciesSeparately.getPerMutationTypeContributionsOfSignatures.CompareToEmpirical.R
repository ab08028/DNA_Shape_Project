...
require(tidyverse)
require(reshape2)
wd="/Users/annabelbeichman/Documents/UW/DNAShapeProject/results/SigProfilerExtractor/20210809_SigProfilerExtractorResults_mice_kSFSAllFreqsSep_mm10GenomeRef/SBS96/"
...
plot1 <- ggplot(cosmic_cominbed_summed,aes(x=frequency,y=proportionOfSites,color=species,group=species))+
...
plot2 <- ggplot(cosmic_cominbed_summed,aes(x=alleleFrequency,y=proportionOfSites,color=species,group=species))+
  facet_wrap(~centralMutationType~signature,scales="free", ncol=2)+
  geom_line()+
  theme_bw()
plot2
...
sum(empirical_melt_summed[empirical_melt_summed$species=="Mmd",]$totalSites)
head(cosmic_activities)
######### plot with empirical ########
...
plot5 <- ggplot(cosmic_cominbed_summed_OverSignatures,aes(x=alleleFrequency,y=proportionOfSites,color=species,group=species,linetype="sigprofilerextractor"))+
...
sum(cosmic_cominbed_summed_OverSignatures[cosmic_cominbed_summed_OverSignatures$species=="Mmd" & cosmic_cominbed_summed_OverSignatures$frequency==1,]$proportionOfSites)
...
plot4_denovo <- ggplot(denovo_combined_summed_OverSignatures,aes(x=alleleFrequency,y=proportionOfSites,color=species,group=species,linetype="sigprofilerextractor"))+
  ...
  theme_bw()
plot4_denovo
ggsave(paste0(plotdir,"model.MutationsContributedPerCentralType.allSigsCombined.alleleFreqs.WITHEMPIRICAL.DeNovoOnly.NoCosmicDecomposition.png"),plot4_denovo,width=12,height=7)
