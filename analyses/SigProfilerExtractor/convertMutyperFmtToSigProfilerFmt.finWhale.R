...
indir="/Users/annabelbeichman/Documents/UW/WhaleProject/fin_whales/results/mutyper/downloaddate_20210111/mutyperResults_20210804_NOSTRICT_3mer/summaryTables/" # mouse 3mers with repeat masking
# note for vaquita strict vs nostrict doesn't matter bc no lower case letters
input <- read.table(paste0(indir,"FinWhale.3merSpectrum.nostrict.SummedOverAllIntervals.NOTEONLYNEUTRALREGIONS.HIGHLYFILTERED.NOTCOMPARABLETOOTHERCETACEANSASIS.txt"),header=T)
head(input)
...
head(input)
names(input)
input_wider <- pivot_wider(select(input,-c(variable,fractionOfAllSegSites)),id_cols = c(`Mutation Types`,population),names_from =  population,values_from = totalSitesAllIntervals)
head(input_wider)                    


# 
write.table(input_wider,paste0(indir,"finWhale.All_Intervals_Counts.perPopulation.nostrict.NOTEONLYNEUTRALREGIONS.HIGHLYFILTERED.ALLFREQS.CentralAsRevCompedToTs.SIGPROFILEREXTRACTORFORMAT.txt"),row.names=F,quote=F,sep="\t")
