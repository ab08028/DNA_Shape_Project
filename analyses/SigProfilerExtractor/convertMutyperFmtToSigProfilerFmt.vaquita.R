...
indir="/Users/annabelbeichman/Documents/UW/WhaleProject/vaquita/results/mutyper/downloadDate_20210216/mutyperResults_20210216/summaryTables/" # mouse 3mers with repeat masking
# note for vaquita strict vs nostrict doesn't matter bc no lower case letters
input <- read.table(paste0(indir,"All_Intervals_CountsProps.ALLFREQS.txt"),header=T)
head(input)
...
head(input)
input_wider <- pivot_wider(select(input,-c(variable,proportionOfSites)),id_cols = c(`Mutation Types`,population),names_from =  population,values_from = value)
head(input_wider)                    


# 
write.table(input_wider,paste0(indir,"vaquita.All_Intervals_Counts.perPopulation.ALLFREQS.CentralAsRevCompedToTs.SIGPROFILEREXTRACTORFORMAT.txt"),row.names=F,quote=F,sep="\t")
