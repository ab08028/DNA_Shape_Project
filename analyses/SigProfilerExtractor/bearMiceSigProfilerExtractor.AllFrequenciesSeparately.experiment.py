...
reference_genome='mm10' # either mouse or human are the only ones supported. run with both? supposed to not matter but still says the genome in the
# run information -- GRCh37 or mm10 
# note this makes a real difference in what signatures are decomposed into (the mouse or human versions)
# combined bear and mouse SFS spcetrum: 
matrixFile="/Users/annabelbeichman/Documents/UW/DNAShapeProject/results/SigProfilerExtractor/input_data/20210825.combined.Bear.Mice.Vaquita.FinWhales.SpectrumPerFrequency.ForSigProfilerExtractor.txt"
# https://osf.io/t6j7u/wiki/2.%20Quick%20Start%20Example/
label="bearsMiceVaquitaFinWhales_kSFSAllFreqsSep_"+reference_genome+"GenomeRef"
...
sig.sigProfilerExtractor(input_type="matrix",output=outdir, input_data=matrixFile, reference_genome= reference_genome, minimum_signatures=1, maximum_signatures=5, nmf_replicates=500, cpu=-1)
# trying with mm10 instead 
# seems like ref genome doesn't matter but run with both genomes just in case
# ref genome does matter for cosmic decomposition actually-- mouse SBS1 image is diff from human SBS1 images (even though same online with cosmic, so that's odd)